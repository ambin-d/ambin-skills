...
## 本地化适配（Agent A版 v1.0，2026-08-09）

本 skill 由 OYYH-Apple/video-storyboard-generator 原版改造，适配Agent A操作方式：

- **用户画像路径**：原版存 `~/.video-storyboard/user-profile.md`（用户主目录，与记忆体系分叉）→ **改指** `<VAULT>\00-系统与规则\video-storyboard\user-profile.md`（画像入库 Vault，与Agent A记忆体系同区，备份随 Vault 走）
- **分镜产出**：原版存「当前项目 /docs」（悬空）→ **默认存** `<VAULT>\收件箱\分镜稿\YYYYMMDD_主题_分镜.md`（存稿入库，可追溯，可复读）
- **OpenCode 执行提示词**：当前环境无 OpenCode，此项降级为可选，产出仅作存稿备用，不作为承诺项
- **Seedance 2.0 提示词**：当前环境无 AI 视频生成工具，提示词按需生成并随分镜存稿，将来喂任何 AI 视频工具均可复用，不悬空承诺
- 其余工作流（头脑风暴/15秒规则/逻辑检查/评分）保持原版能力不变
...
- **Agent A专用路径**: `<VAULT>\00-系统与规则\video-storyboard\user-profile.md`
...
**如果画像文件不存在：**
使用默认设置，并在本次交互结束后询问是否保存偏好（保存位置同上，写入 Vault）。
...
9. **OpenCode提示词** - 需要生成可在OpenCode中使用的执行提示词吗？（仅专业版）
    - 是：生成可直接复制到OpenCode的技术实现提示词（**当前环境无 OpenCode，产出仅作存稿备用**）
    - 否：跳过，仅输出分镜文档
...
10. **Seedance 2.0 提示词** - 需要生成可用于Seedance 2.0 AI视频生成工具的提示词吗？
    - **是**：为每个镜头生成符合Seedance 2.0格式的提示词，包含：
      ...
      - [其他控制]：时长、分辨率、质量要求
    - **否**：跳过Seedance提示词生成
    - **注**：当前环境无 AI 视频生成工具，生成的提示词随分镜存稿，将来可复用
...
**（Agent A 适配：当前环境无 OpenCode，产出仅作存稿备用，不作为承诺项。）**
...
- **（Agent A 适配：提示词随分镜存稿，将来接入 AI 视频工具时直接复用。）**
...
**默认位置（Agent A 适配）：** `<VAULT>\收件箱\分镜稿\YYYYMMDD_主题_分镜.md`

- 文件名按「日期_主题_分镜」命名（如 `20260809_选题X_分镜.md`）
- 存稿入库 Vault，可追溯、可复读、可被知识库检索
- 生成后向用户报告完整路径和文件要点
- 若用户指定其他位置，按用户指定保存
...
分镜生成完成后，更新用户画像文件（`<VAULT>\00-系统与规则\video-storyboard\user-profile.md`）。记录本次交互中用户的选择偏好，详见 `references/user-profile.md`。
...
- `config/user-profile-template.md` - 用户画像模板（使用时写入 `<VAULT>\00-系统与规则\video-storyboard\user-profile.md`）
...
### 用户系统
- `references/user-profile.md` - 用户画像机制（路径已适配 Vault）
...
2. **OpenCode 执行提示词（可选）** - 可直接用于执行视频制作（当前环境无 OpenCode，存稿备用）
3. **保存位置确认** - 分镜文档默认存入 `<VAULT>\收件箱\分镜稿\`，确保文档妥善保存
