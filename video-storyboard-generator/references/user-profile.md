...
- 原版画像存用户主目录 `~/.video-storyboard/user-profile.md`，与记忆体系分叉
- 本版改存 Vault：`<VAULT>\00-系统与规则\video-storyboard\user-profile.md`
- 画像随 Vault 备份、可追溯，与Agent A记忆体系同区
- 分镜产出默认存 `<VAULT>\收件箱\分镜稿\`（见 SKILL.md 阶段6）
...
**存储位置：** 
- **Agent A专用**: `<VAULT>\00-系统与规则\video-storyboard\user-profile.md`
...
1. 删除画像文件：删除 `<VAULT>\00-系统与规则\video-storyboard\user-profile.md`
2. 在对话中请求重置："重置我的偏好设置"
...
def load_user_profile():
    # Agent A专用路径：画像入库 Vault
    profile_path = r"<VAULT>\00-系统与规则\video-storyboard\user-profile.md"
